...
interface Rec { id: string; category: string; title: string; description: string; reasoning: string; difficulty: string; impact: string; estimatedReduction: number; accepted: boolean; confidenceScore: number; }

const fetcher = (url: string) => fetch(url).then(res => res.json()).then(j => j.data ?? []);

export default function RecommendationsPage() {
  const { data: recs = [], error, isLoading: loading, mutate } = useSWR<Rec[]>("/api/recommendations", fetcher);
  const [gen, setGen] = useState(false);
  const [exp, setExp] = useState<string | null>(null);

  async function genNew() { setGen(true); try { const r = await fetch("/api/recommendations", { method: "POST" }); if (r.ok) { mutate(); } } catch(e) { console.error(e); } finally { setGen(false); } }
  async function accept(id: string) { try { await fetch("/api/recommendations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "accept", recommendationId: id }) }); mutate(recs.map(r => r.id === id ? { ...r, accepted: true } : r), false); } catch(e) { console.error(e); } }
